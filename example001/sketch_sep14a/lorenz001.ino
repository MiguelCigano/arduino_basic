...
void loop() {
  ...
  int xPixel = map(x * 10, -30, 30, 0, TFT_WIDTH);
  int yPixel = map(y * 10, -30, 30, 0, TFT_HEIGHT);

  // Dibuja el punto en la pantalla
  uint16_t color = tft.color565((int)(z * 10) % 255, 100, 200); // Color dinámico según z
  tft.drawPixel(xPixel, yPixel, color);

  delay(10); // Pequeño retraso para suavizar el efecto
}
